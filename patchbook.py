...
def initial_print():
    printerr()
    printerr("██████████████████████████████")
    printerr("       PATCHBOOK PARSER       ")
    printerr("   Created by Spektro Audio   ")
    printerr("██████████████████████████████")
    printerr()
    printerr("Version " + parserVersion)
    printerr()
...
def parseFile(filename):
    ...
    lines = []
    try:
        printerr("Loading file: " + filename)
        with open(filename, "r") as file:
            for l in file:
                lines.append(l)
                regexLine(l)
    except TypeError:
        printerr("ERROR. Please add text file path after the script.")
    except FileNotFoundError:
        printerr("ERROR. File not found.")
    printerr("File successfully processed.")
    printerr()
...
def regexLine(line):
    ...
    # CHECK FOR VOICES
    if debugMode:
        printerr("Cheking input for voices...")
    ...
    try:
        ...
        if "*" not in results and "-" not in results and "|" not in results:
            if debugMode:
                printerr("New voice found: " + results.upper())
    ...
    if debugMode:
        printerr("Checking input for connections...")
    ...
    try:
        ...
        if len(results) == 6:
            if debugMode:
                printerr("New connection found, parsing info...")
    ...
    # CHECK PARAMETERS
    if debugMode:
        printerr("Checking for parameters...")
    ...
    try:
        ...
        if debugMode:
            printerr("New module found: " + module)
        if results[1] != None:
            ...
            if debugMode:
                printerr("No parameters found. Storing module as global variable...")
    ...
    if "|" in line and "=" in line and "*" not in line:
        ...
        if debugMode:
            printerr("Using global variable: " + module)
...
def parseArguments(args):
    ...
    if debugMode:
        printerr("Parsing arguments: " + args)
    ...
    if debugMode:
        printerr("All arguments processes.")
...
def addConnection(list, voice="none"):
    ...
    if debugMode:
        printerr("Adding new connection...")
    if debugMode:
        printerr("-----")
    ...
    if debugMode:
        printerr("Output module: " + output_module)
    if debugMode:
        printerr("Output port: " + output_port)

    try:
        connection_type = connectionTypes[list[2].lower()]
        if debugMode:
            printerr("Matched connection type: " + connection_type)
    except KeyError:
        printerr("Invalid connection: " + list[2])
        connection_type = "cv"
    ...
    if debugMode:
        printerr("Input module: " + input_module)
    if debugMode:
        printerr("Input port: " + output_port)

    checkModuleExistance(output_module, output_port, "out")
    checkModuleExistance(input_module, input_port, "in")

    if debugMode:
        printerr("Appending output and input connections to mainDict...")
...
def checkModuleExistance(module, port="port", direction=""):
    ...
    if debugMode:
        printerr("Checking if module already existing in main dictionary: " + module)
...
def addParameter(module, name, value):
    ...
    if debugMode:
        printerr("Adding parameter: " + module + " - " + name + " - " + value)
...
def doCommand(command):
    if command == "module":
        ...
        printerr("Invalid command, please try again.")

def askCommand():
    doCommand(input("> ").lower().strip())
    askCommand()
...
def detailModule():
    ...
    if module in mainDict["modules"]:
        printerr("-------")
        printerr("Showing information for module: " + module.upper())
        printerr()
        printerr("Inputs:")
        for c in mainDict["modules"][module]["connections"]["in"]:
            keyvalue = mainDict["modules"][module]["connections"]["in"][c]
            printerr(keyvalue["output_module"].title() + " (" + keyvalue["output_port"].title(
        ...
        for x in mainDict["modules"][module]["connections"]["out"]:
            ...
            for c in port:
                keyvalue = c
                printerr(x.title() + " > " + keyvalue["input_module"].title() + " (" + keyvalue["input_port"].title(
...
def printConnections():
    printerr()
    printerr("Printing all connections by type...")
    printerr()

    for ctype in connectionTypes:
        ctype_name = connectionTypes[ctype]
        printerr("Connection type: " + ctype_name)
        # For each module
...
def exportJSON():
    ...
    printerr("Exporting dictionary as file: " + filepath)
...
def graphviz():
    ...
    printerr("Generating signal flow code for GraphViz.")
    printerr("Copy the code between the line break and paste it into https://dreampuf.github.io/GraphvizOnline/ to download a SVG / PNG chart.")
    conn = []
    total_string = ""
    printerr("-------------------------")
    print("digraph G{\nrankdir = LR;\nsplines = spline;\nordering=out;")
    ...
    for module in sorted(mainDict["modules"]):
        ...
        if os.path.isfile(template_file):
            ...
            line_nr = 1
            if line != "ports:":
                printerr("ERROR parsing template " + template_file + " (line " + str(line_nr) + ")"
                         + ": expected 'ports:'")
                sys.exit(1)
            line = next(lines)
            line_nr += 1
            ...
            for inp in inputs:
                ...
                if not inp in ports:
                    printerr("ERROR: port '" + inp + "' is not defined in template " + template_file)
                    sys.exit(1)
            for out in outputs:
                out = re.sub('[^A-Za-z0-9]+', '', out)
                if not out in ports:
                    printerr("ERROR: port '" + out + "' is not defined in template " + template_file)
                    sys.exit(1)
            params = mainDict["modules"][module]["parameters"]
            for param in params:
                if not param in supported_params:
                    printerr("ERROR: parameter '" + param + "' is not defined in template " + template_file)
                    sys.exit(1)
                param_value = params[param]
                if not re.match(supported_params[param], param_value):
                    printerr("ERROR: invalid parameter '" + param + " = " + param_value
                ...
                def fn(match):
                    ...
                    if match.group(1):
                        if tag == "curdir":
                            ...
                            printerr("ERROR parsing template " + template_file + ": can not render {{" + tag + "}}: "
                        ...
                        if tag == "render_svg":
                            ...
                            if debugMode:
                                printerr('Running ImageMagick: ' + str(cmd))
                            proc = subprocess.Popen(cmd, cwd=templates_dir)
                            proc.wait()
                            if proc.returncode == 0:
                                return png_file_name
                            else:
                                printerr("ERROR rendering template " + template_file + ": SVG could not be rendered:\n\n" + content)
                                sys.exit(1)
                        elif tag in lambdas:
...
def printDict():
    ...
    for key in mainDict["modules"]:
        printerr(key.title() + ": " + str(mainDict["modules"][key]))
